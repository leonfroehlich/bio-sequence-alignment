...
def test_alignment(case_id, seq1, seq2, exp_a1, exp_a2, exp_score):
    alignment = aligner.global_alignment(seq1, seq2)
    assert alignment.score == exp_score, f"Case {case_id}: score mismatch"
    assert alignment.aligned_seq1 == exp_a1, f"Case {case_id}: aligned_seq1 mismatch"
    assert alignment.aligned_seq2 == exp_a2, f"Case {case_id}: aligned_seq2 mismatch"